...
def load_data():
    ...
    df_agg = pd.read_csv('D:\works\Streamlit\Aggregated_Metrics_By_Video.csv').iloc[1:,:]
    df_agg.columns = ['Video','Video title','Video publish time','Comments added','Shares','Dislikes','Likes',
                      'Subscribers lost','Subscribers gained','RPM(USD)','CPM(USD)','Average % viewed','Average view duration',
                      'Views','Watch time (hours)','Subscribers','Your estimated revenue (USD)','Impressions','Impressions ctr(%)']
    df_agg['Video publish time'] = pd.to_datetime(df_agg['Video publish time'], errors='coerce', format='mixed')
    df_agg['Average view duration'] = df_agg['Average view duration'].apply(lambda x: datetime.strptime(x,'%H:%M:%S'))
    df_agg['Avg_duration_sec'] = df_agg['Average view duration'].apply(lambda x: x.second + x.minute*60 + x.hour*3600)
    df_agg['Engagement_ratio'] =  (df_agg['Comments added'] + df_agg['Shares'] +df_agg['Dislikes'] + df_agg['Likes']) /df_agg.Views
    df_agg['Views / sub gained'] = df_agg['Views'] / df_agg['Subscribers gained']
    df_agg.sort_values('Video publish time', ascending = False, inplace = True)    
    df_agg_sub = pd.read_csv('D:\works\Streamlit\Aggregated_Metrics_By_Country_And_Subscriber_Status.csv')
    df_comments = pd.read_csv('D:\works\Streamlit\Aggregated_Metrics_By_Video.csv')
    df_time = pd.read_csv('D:\works\Streamlit\Video_Performance_Over_Time.csv')
...
for i in metric_medians6mo.index:
    with columns[count]:  # 确保这个块被正确缩进
        # 计算 delta
        delta = (metric_medians6mo[i].timestamp() - metric_medians12mo[i].timestamp()) / metric_medians12mo[i].timestamp()
        
        # 显示 metric，舍入值
        st.metric(label=i, value=round(metric_medians6mo[i].timestamp(), 1), delta="{:.2%}".format(delta))
        
        count += 1  # 计数器增加
        if count >= 5:  # 如果 count 超过 5，则重置为 0
            count = 0
